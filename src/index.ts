...
export async function startServer(options?: StartOptions): Promise<ServerHandle> {
  ...
  const apiCallLogs = new ApiCallLogStore();
  const chatRoutes = createChatRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, apiCallLogs);
  const messagesRoutes = createMessagesRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, apiCallLogs);
  const geminiRoutes = createGeminiRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, apiCallLogs);
  const responsesRoutes = createResponsesRoutes(accountPool, cookieJar, proxyPool, upstreamRouter, apiCallLogs);
  ...
  const webRoutes = createWebRoutes(accountPool, usageStats, apiCallLogs);
