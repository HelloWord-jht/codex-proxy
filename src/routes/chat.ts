...
export function createChatRoutes(
  ...
  app.post("/v1/chat/completions", async (c) => {
    ...
    if (upstreamRouter && !upstreamRouter.isCodexModel(req.model)) {
      ...
      return handleDirectRequest(c, upstreamRouter.resolve(req.model), directReq, fmt, apiCallLogs);
    }

    return handleProxyRequest(c, accountPool, cookieJar, proxyReq, fmt, proxyPool, apiCallLogs);
  });
