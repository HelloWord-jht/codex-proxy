...
export class GeminiUpstream implements UpstreamAdapter {
  ...
  getRequestLogInfo(req: CodexResponsesRequest): ApiCallRequestInfo {
    const modelId = extractModelId(req.model);
    return {
      interfaceIdentifier: "gemini.streamGenerateContent",
      interfaceName: "Gemini streamGenerateContent",
      interfaceUrl: `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(modelId)}:streamGenerateContent`,
    };
  }
