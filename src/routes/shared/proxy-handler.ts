  ...
  for (;;) {
    try {
      ...
      if (req.isStreaming) {
        ...
        return stream(c, async (s) => {
          ...
          try {
            ...
            if (capturedLogId) {
              apiCallLogs?.completeCall(capturedLogId, usageInfo
                ? {
                    status: "success",
                    is_success: true,
                ...
                : {
                    ...
                    is_success: false,
                    error_message: abortController.signal.aborted
                      ? "Client aborted the streaming response"
      ...
      return await handleNonStreaming(
        ...
        codexApi, rawResponse, entryId, abortController, released, affinityMap, conversationId, upstreamTurnState, apiCallLogs, callLogId,
      );
    } catch (err) {
      if (!(err instanceof CodexApiError)) {
        if (callLogId) {
          apiCallLogs?.completeCall(callLogId, {
            ...
            error_message: err instanceof Error ? err.message : "Unknown proxy error",
          });
        }
      ...
      if (collectErr instanceof EmptyResponseError && attempt <= MAX_EMPTY_RETRIES) {
        ...
        if (callLogId) {
          apiCallLogs?.completeCall(callLogId, {
            status: "failed",
            is_success: false,
            input_tokens: collectErr.usage?.input_tokens ?? 0,
            output_tokens: collectErr.usage?.output_tokens ?? 0,
            error_message: collectErr.message,
          });
        }
        ...
        try {
          ...
          if (callLogId) {
            apiCallLogs?.completeCall(callLogId, {
              status: abortController.signal.aborted ? "aborted" : "failed",
              is_success: false,
              error_message: retryErr instanceof Error ? retryErr.message : "Retry request failed",
            });
          }
