...
import type { ApiCallRequestInfo } from "../services/api-call-logs.js";
...
export class CodexApi {
  ...
  getRequestLogInfo(request: CodexResponsesRequest | CodexCompactRequest): ApiCallRequestInfo {
    const baseUrl = this.resolveBaseUrl().replace(/\/$/, "");
    if ("instructions" in request && !("stream" in request)) {
      return {
        interfaceIdentifier: "codex.responses.compact",
        interfaceName: "Codex Responses Compact",
        interfaceUrl: `${baseUrl}/codex/responses/compact`,
      };
    }

    return {
      interfaceIdentifier: request.useWebSocket ? "codex.responses.websocket" : "codex.responses",
      interfaceName: request.useWebSocket ? "Codex Responses WebSocket" : "Codex Responses API",
      interfaceUrl: `${baseUrl}/codex/responses`,
    };
  }
