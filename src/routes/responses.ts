...
async function handleCompact(
  ...
  let callLogId = apiCallLogs?.startCall({
    ...codexApi.getRequestLogInfo(compactRequest),
      ...
      if (!(err instanceof CodexApiError)) {
        if (callLogId) {
          apiCallLogs?.completeCall(callLogId, {
            ...
            is_success: false,
            error_message: err instanceof Error ? err.message : "Unknown compact error",
          });
        }
        releaseAccount(accountPool, entryId, undefined, released);
        throw err;
...
export function createResponsesRoutes(
  ...
  const responsesHandler = async (c: Context) => {
    ...
    if (upstreamRouter && !upstreamRouter.isCodexModel(rawModel)) {
      ...
      return handleDirectRequest(c, upstreamRouter.resolve(rawModel), directReq, PASSTHROUGH_FORMAT, apiCallLogs);
    }

    return handleProxyRequest(c, accountPool, cookieJar, proxyReq, PASSTHROUGH_FORMAT, proxyPool, apiCallLogs);
  };
  ...
  const compactHandler = async (c: Context) => {
    ...
    return handleCompact(c, accountPool, cookieJar, proxyPool, body, apiCallLogs);
  };
