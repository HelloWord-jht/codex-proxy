...
export function createGeminiRoutes(
  ...
  app.post("/v1beta/models/:modelAction", async (c) => {
    ...
    if (upstreamRouter && !upstreamRouter.isCodexModel(geminiModel)) {
      ...
      return handleDirectRequest(c, upstreamRouter.resolve(geminiModel), directReq, GEMINI_FORMAT, apiCallLogs);
    }

    return handleProxyRequest(c, accountPool, cookieJar, proxyReq, GEMINI_FORMAT, proxyPool, apiCallLogs);
  });
